...
We have cleaned the data for you and will show you a snippet of what it's structure looks like. Based on that structure, in groups of three:
...
ggplot(hatedata, aes(x=____, y=_____)) + ______
...
head(as_tibble(puldata))
#as_tibble(puldata) %>% knitr::kable(digits = 5, caption = "Pulitzer and Newspaper data")
...
head(as_tibble(trumpdata))
#as_tibble(trumpdata) %>% knitr::kable(digits = 5, caption = "Trump Twitter data")
...
head(as_tibble(hiphopdata))
#as_tibble(hiphopdata) %>% knitr::kable(digits = 5, caption = "Hiphop Politics Data")
...
head(as_tibble(commaperc))
#as_tibble(commaperc) %>% knitr::kable(digits = 5, caption = "Oxford Comma Opinions")
...
head(as_tibble(airdata))
#as_tibble(airdata) %>% knitr::kable(digits = 5, caption = "Airport fatality data")
```
...
head(as_tibble(drugdata))
#as_tibble(drugdata) %>% knitr::kable(digits = 5, caption = "Drug use data")
...
```{r, eval=FALSE}
ggplot(_____, aes(x=____ , group=____, y=______)) + geom_____(___________)
...
head(as_tibble(biopicdata))
#as_tibble(biopicdata) %>% knitr::kable(digits = 5, caption = "Biopic subject sex data")
...
```{r, eval=FALSE}
ggplot(_______, aes(x=_______, fill=______)) + geom_____(____) 
...
head(as_tibble(map2))
#as_tibble(map2) %>% knitr::kable(digits = 5, caption = "Map data")
```
...
head(as_tibble(ladata))

#as_tibble(ladata) %>% knitr::kable(digits = 5, caption = "Love Actually data")
```
