...
* **The slides, in [.pdf](files/quant-data-vis/Quantitative_Data_Visualisation.pdf) or [.pptx](files/quant-data-vis/Quantitative_Data_Visualisation.pptx) format** - this page includes the same information, but you might prefer to follow along with the slides
...
Here's a really cool fun fact! The logo for Hack Your Data Beautiful (as proudly displayed on the [home page](index.html)) was in fact made in `ggplot2`, with the `geom_density()` function! Here's some reproducible code that we used to generate it:
